...
%% BEFORE RUNNING THE CODE CHANGE NAME TO upd_step
    ...
    R_t = diag([ones(3,1)*1e-3;ones(3,1)*1e-3;ones(3,1)*1e-3]);
C = [zeros(3) zeros(3) zeros(3) zeros(3) zeros(3);zeros(3) zeros(3) zeros(3) zeros(3) zeros(3);zeros(3) zeros(3) eye(3) zeros(3) zeros(3)];
K_t = (covarEst*C')*(inv(C*covarEst*C' + R_t));
uCurr = uEst + K_t*([zeros(6,1);z_t] - C * uEst);
